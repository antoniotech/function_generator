...
void *ioProcessing(void *envByRef, void *apPtr){
    ...
    *volume = (*envPtr).volume;
    ...
    dspBlockProcess((short *)(*ap).outputBuffer, (short *)(*ap).outputBuffer, (*ap).blksize/2, filter_on, volume );
    ...
    while( !(*envPtr).quit ) {
		...
		*volume = (*envPtr).volume;
		...
		dspBlockProcess((short *)(*ap).outputBuffer, (short *)(*ap).inputBuffer, (*ap).blksize/2, filter_on, volume);
